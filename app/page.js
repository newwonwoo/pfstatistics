...
export default function Home() {
  ...
  */
  const gate = useMemo(() => {
    ...
    */
    /*
      **수기입력 행이 남 얘기를 하고 있었다**(실측 2026-09-25) —
      「교통환경 — [교통환경] 탭에서 시설을 수집하세요」 는 바로 윗줄(반경시설 수집)의 사유인데
      점수표의 **빈 행 전부**를 세는 바람에 여기로 딸려왔다. 그 탭에서 할 수 없는 일을
      그 탭 줄에 적으면 가서도 할 것이 없다.
      **여기서 손으로 넣는 행(`kind: 'form'`)만** 센다. 자동 항목이 비어 있는 것은
      ...
      {coord && gatherTab && (
        <button style={S.mapToggle} onClick={() => setMapOpen(o => !o)}>
          ...
          <span style={S.mapToggleNote}>
            ...
            */}
            {/*
              **이미 그리고 있는데도 "누르면 여기서 그립니다" 라고 적혀 있었다**(실측 2026-09-24).
              [반경시설 수집] 을 눌러 그리기 모드로 들어온 뒤에도 손잡이 문구가 그대로라,
