...
export default function ReviewView({ region, addr, facilities, compare, rate, value, onChange }) {
  const v = value ?? {};
  const set = (patch) => onChange?.({ ...v, ...patch });
  const put = (id, x) => set({ [id]: x });

  /* 초기예상분양률은 그 탭과 **같은 함수**로 낸다 — 두 화면의 숫자가 갈리면 안 된다 */
  const { total: rateTotal, res } = useMemo(() => expectedRateOf(compare, rate), [compare, rate]);
