...
- **평가기준 구간표는 받는 대로 하나씩 적용**한다 (`config/scoring.json` 에만 둔다).
  ...
  아직 없는 것: **지하철역의 "존재" 구간**, 그리고 **공원·문화·공공의 "1km 3개 모두 존재" 한 행**.
  둘 다 받은 캡쳐가 그 행에서 잘려 있다(사용자: "내일 제대로 줄게" 2026-09-14).
  빗금 유지 — 추정해서 채우지 말 것.
  **지하철역의 부재행은 골든 캡쳐로 확정했다** — `사업지 반경 1km이내 부재 = 1점 매우열악`
  (`docs/evidence-samples/01_교통환경.jpg`). 역이 잡히면 점수를 비우고 "구간표 미수령" 을 적는다.
