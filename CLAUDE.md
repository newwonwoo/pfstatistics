...
- **심사평점표 = 최종 산출물**(2026-09-16). 구조가 드러났다.
  ...
  **이 앱이 만든 초기예상분양률이 심사평점표의 「초기분양률(22)」 항목이 되어 최종 평점에 들어간다.**
  · 초기분양률 배점표(완전 수령) : 100% 22 / 90~100 21 / 80~90 19 / 70~80 16 / 60~70 13 / 50~60 10 / 50미만 0
    골든 검산 70% → **16점**. 탄벌A지구 평점표 재현 합계 **94점**.
  · **0점 처리 규칙을 자동으로 건다** — 초기분양률 50% 미만 **또는** 누적DSCR 1.00 미만이면
    **두 항목 평점을 모두 0점**. 실무자가 잊기 쉬운 연동이라 화면이 대신 걸되 배지·배너로 남긴다.
  · **사업성은 수동입력**(사용자 결정 2026-09-16) — 사업수익률·누적DSCR·자기자금 투입규모는
    사업수지표에서 나오는 값이라 이 앱 원천에 없다. 시공자 항목도 구간표를 다 못 받아 수동입력.
    아는 구간(13.5%↑=20 · 1.05↑=3 · 8%↑=20 · 1~20위=15 · AAA/AA=15 · 50%미만=5)은 근거 칸에 적어 둔다.
  · 사업수익률의 **분양가 = Min(적정분양가, 예정분양가)** — 적정분양가를 이 앱의 비교사업장 탭이 낸다.
    공사비 = Max(공사비 최저기준, 도급계약서상 공사비).
  · **배점 판독 주의** : 실제 평점표 캡쳐는 합계 100 · 사업수지분석 65 · 시공자 15/15/5(=35) 다.
    설명표 캡쳐의 괄호 숫자(5/15/10)는 흐려 다르게 읽혔다 — **합계가 맞는 실제 평점표를 따랐다.**

## 미결 사항

- **평가기준 구간표는 받는 대로 하나씩 적용**한다 (`config/scoring.json` 에만 둔다).
  아직 없는 것: **지역경쟁력(매매지수 증감률) 상위 구간** · **소비심리지수 구간표** ·
  **심사평점표의 종합평점→심사등급→보증료율 구간표**(86점이상=1등급·0.563% 한 줄만 보인다) ·
  심사평점표 각 항목의 하위 구간(최고점 행만 보인다).
