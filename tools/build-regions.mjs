...
/**
 * 통계누리는 시도를 축약형으로 준다. 정식 명칭 표.
 *
 * 행정구역은 실제로 바뀐다 — 2026 기준 **전라남도+광주광역시가 통합**됐다.
 * 통계누리는 "전남광주", KB 는 "광주(코드 12)" 로 부르고 옛 것을 "(구)광주광역시"
 * "(구)전라남도" 로 남겨뒀다. 카카오 주소검색이 돌려주는 정식 명칭은
 * **전남광주통합특별시** 다(실측). 옛 이름도 남겨둬야 과거 자료를 읽을 수 있다.
 */
const SIDO = {
